...
#     "pyarrow>=24.0.0",
#     "altair>=6.1.0",
#    "openhound-kubernetes",
# ]
#
# [tool.uv.sources]
# openhound = { git = "https://github.com/specterops/OpenHound", branch = "feature/notebooks" }
# openhound-kubernetes = { path = "..", editable = true }
#
# ///
...
def _(dlt_pipeline, mo, selected_schema):
    ...
    mo.stop(
        not dataset_tables, f"Schema '{selected_schema.value}' has no resource tables"
    )
...
def _(collector_options, matched_extension_name, mo):
    mo.stop(
        not matched_extension_name, "No loaded extension matches the selected schema"
    )
...
def _(
    ...
    def as_node_preview(row, model):
        ...
        return {
            **node_dict,
            **{f"prop_{key}": value for key, value in properties.items()},
        }

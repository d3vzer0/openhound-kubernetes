...
@app.asset(
    description="Kubernetes DaemonSet asset. Contains no direct edges, but serves as a parent for all pods created by the DaemonSet.",
    node=NodeDef(
        properties=K8SNodeProperties,
        kind=nk.DAEMON_SET,
        description="Kube DaemonSet",
        icon="gears",
        color="#68bc00",
    ),
    edges=[
...
class DaemonSet(BaseAsset):
    ...
    def as_node(self) -> "K8SNode":
        ...
        return K8SNode(kinds=[nk.DAEMON_SET], properties=properties)
